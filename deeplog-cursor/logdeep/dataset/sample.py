...
def sliding_window(csv_path, window_size, sample_ratio=1):
    '''
    dataset structure
        result_logs(dict):
    ...
    csv_path: absolute or relative path to preprocessed CSV (must contain EventId column).
    '''
    result_logs = {}
...
# Write by myself
    ...
    print('File {}, number of seqs {}'.format(csv_path,
                                              len(result_logs['Sequentials'])))
    print('File {}, number of labels {}'.format(csv_path,
                                              len(labels)))
